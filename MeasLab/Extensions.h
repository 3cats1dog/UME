...
namespace MeasLab {
    ...
    public:
        ...
        static int GetMagValue(Decimal d )
        {
            return GetMagValue((double)d);
        }
        static int GetMagValue(double d)
        {
            double exponent = Math::Log10(Math::Abs(d));
            if (Math::Abs(d) >= 1)
            {
                ...
                case 1:
                case 2:
                    return  0;    // d.ToString(format, trcul) + " " + unit;
                case 3:
                case 4:
                case 5:
                    return 3;   // (d / 1e3).ToString(format, trcul) + " k" + unit;
                case 6:
                case 7:
                case 8:
                    return 6;   // (d / 1e6).ToString(format, trcul) + " M" + unit;
                case 9:
                case 10:
                case 11:
                    return 9;   // (d / 1e9).ToString(format, trcul) + " G" + unit;
                case 12:
                case 13:
                case 14:
                    return 12;  // (d / 1e12).ToString(format, trcul) + " T" + unit;
                case 15:
                case 16:
                case 17:
                    return 15;  // (d / 1e15).ToString(format, trcul) + " P" + unit;
                case 18:
                case 19:
                case 20:
                    return 18;  // (d / 1e18).ToString(format, trcul) + " E" + unit;
                case 21:
                case 22:
                case 23:
                    return 21;  // (d / 1e21).ToString(format, trcul) + " Z" + unit;
                default:
                    return 24;  // (d / 1e24).ToString(format, trcul) + " Y" + unit;
                }
            }
            else if (Math::Abs(d) > 0)
            {
                ...
                case -2:
                case -3:
                    return -3;  // (d * 1e3).ToString(format, trcul) + " m" + unit;
                case -4:
                case -5:
                case -6:
                    return -6;  // (d * 1e6).ToString(format, trcul) + L" μ" + unit;
                case -7:
                case -8:
                case -9:
                    return -9;  // (d * 1e9).ToString(format, trcul) + " n" + unit;
                case -10:
                case -11:
                case -12:
                    return -12; // (d * 1e12).ToString(format, trcul) + " p" + unit;
                case -13:
                case -14:
                case -15:
                    return -15; // (d * 1e15).ToString(format, trcul) + " f" + unit;
                case -16:
                case -17:
                case -18:
                    return -18; // (d * 1e18).ToString(format, trcul) + " a" + unit;
                case -19:
                case -20:
                case -21:
                    return -21; // (d * 1e21).ToString(format, trcul) + " z" + unit;
